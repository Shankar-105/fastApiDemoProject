...
async def load_missed_content(
    ...
    """Fetch all undelivered messages, shared posts, and notifications for a
    user reconnecting after being offline.

    Uses SELECT ... FOR UPDATE with skip_locked to atomically claim and mark
    unread messages/shares as read, preventing duplicate delivery on concurrent
    reconnects. Returns a time-sorted list of dicts with type discriminators
    ("message", "shared_post", "notification") that the WebSocket handler sends
    to the client before entering the main dispatch loop. Called from chat.py
    immediately after WebSocket authentication.
    """
